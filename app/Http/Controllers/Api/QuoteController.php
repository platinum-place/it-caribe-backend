...
class QuoteController extends Controller
{
    ...
    {
        $fields = ['id', 'Quoted_Items'];
        $quote = $this->crm->getRecords('Quotes', $fields, $request->get('cotz_id'))['data'][0];

        $data = [
            'Depurado' => true,
        ];
        $this->crm->updateRecords('Quotes', $request->get('cotz_id'), $data);
    ...
    public function inspect(InspectRequest $request)
    {
        ...
        foreach ($photos as $photo => $title) {
            ...
            $path = "photos/{$request->get('cotz_id')}/uploads/".date('YmdHis')."/$title.$extension";

            Storage::put($path, $imageData);

            $this->crm->uploadAnAttachment('Quotes', $request->get('cotz_id'), $path);
        }
    ...
    {
        $fields = ['id', 'File_Name'];
        $attachments = $this->crm->attachmentList('Quotes', $request->get('cotz_id'), $fields);

        $response = [];

        foreach ($attachments['data'] as $attachment) {
            $imageData = $this->crm->getAttachment('Quotes', $request->get('cotz_id'), $attachment['id']);
            ...
            $path = "photos/{$request->get('cotz_id')}/downloads/".date('YmdHis')."/{$attachment['File_Name']}.$extension";
